...
class Scene:
    ...
    def save_fsds(self, iteration, stage):
        if stage == "coarse":
            point_cloud_path = os.path.join(self.model_path, "point_cloud_full_sds/coarse_iteration_{}".format(iteration))

        else:
            point_cloud_path = os.path.join(self.model_path, "point_cloud_full_sds/iteration_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.gaussians.save_deformation(point_cloud_path)

    def save_3dedit(self, iteration, stage, run_name):
        if stage == "coarse":
            point_cloud_path = os.path.join(self.model_path, "point_cloud_3dedit/coarse_iteration_{}".format(iteration))

        else:
            point_cloud_path = os.path.join(self.model_path, "point_cloud_3dedit/{}/iteration_{}".format(run_name, iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.gaussians.save_deformation(point_cloud_path)
        
    def save_refine(self, iteration, stage, prompt):
        if stage == "coarse":
            point_cloud_path = os.path.join(self.model_path, f"point_cloud_refine/coarse_iteration_{iteration}")

        else:
            point_cloud_path = os.path.join(self.model_path, f"point_cloud_refine/{prompt}/iteration_{iteration}")
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.gaussians.save_deformation(point_cloud_path)